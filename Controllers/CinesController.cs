...
namespace back_end.Controllers
{
    ...
    public class CinesController:ControllerBase
    {
        ...
        public async Task<ActionResult<List<CineDTO>>> Get([FromQuery] PaginacionDTO paginacionDTO)
        {
            ...
            var cines = await queryable.OrderBy(cine => cine.Nombre).Paginar(paginacionDTO).ToListAsync();
        ...
        {
            var cine = await context.Cines.FirstOrDefaultAsync(cine => cine.Id == Id);
            if (cine == null)
            {
                return NotFound();
            }
        ...
        {
            var cine = await context.Cines.FirstOrDefaultAsync(item => item.Id == Id);
            if (cine == null)
            {
                return NotFound();
            }
